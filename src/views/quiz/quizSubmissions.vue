<template>
	<Index>
		<div class="flex h-full w-full flex-col">
			...
			<div class="flex flex-1 space-x-8 p-8">
				<div class="h-fit justify-between rounded-xl border bg-white">
					...
					<div class="w-[250px] space-y-4 p-4">
						...
						<div class="space-y-1">
							...
							<p
								class="text-right text-xs font-medium text-neutral-700"
							>
								{{ lastSubmissionCreatedAtDateTimeString }}
							</p>
						</div>
					</div>
				</div>
				<div class="flex-1 space-y-8">
					...
					<div class="grid grid-cols-3 gap-4">
						<router-link
							class="block w-full rounded-lg border bg-slate-50 px-6 py-4 transition hover:brightness-95 active:scale-95"
							:to="`/submission/quiz/${quizSubmission.id}`"
							:class="{
								'border-blue-300':
									quizSubmission.attributes.state === 'PASS',
								'border-yellow-400 bg-stripes bg-stripes-yellow-100':
									quizSubmission.attributes.state === 'FAIL',
